    ...
    <div className="h-full w-[70%] flex flex-col bg-gray-50 border-gray-200 border-2 rounded-lg pb-4 shadow-sm overflow-hidden ">
      ...
      <div className="flex items-center justify-between p-4 border-b">
        <div className="flex items-center gap-3">
          <img
            src={currentConversation.otherParticipant?.avatar || userAvatar}
            alt=""
            className="w-12 h-12 justify-center rounded-full object-cover"
          />
          <div>
            <h3 className="font-semibold text-lg">
              {currentConversation.otherParticipant?.pseudo}
            </h3>
            <p className="text-gray-500 text-sm">
              {currentConversation.otherParticipant?.nom}{" "}
              {currentConversation.otherParticipant?.prenom}{" "}
            </p>
          </div>
        </div>
      ...
      {messages.length > 0 ? (
        ...
        >
          {messages.map((msg) => {
            const isUserMessage =
              msg.user_id._id === currentConversation.otherParticipant._id;
            return (
              <div
                key={msg._id}
                className={`flex ${
              ...
              >
                <div className="relative flex flex-col mr-3 ml-3 mb-1">
                  <div
                    className={`p-3 rounded-lg max-w-xs md:max-w-xs ${
                      isUserMessage ? "bg-yellow-50" : "bg-gray-100"
                    }`}
                  >
                    <div className=" max-h-full w-full">
                      {msg.isDeleted ? (
                        msg.fichier ? (
                          ...
                          <span className="text-gray-500 italic mt-2">
                            Message supprimé
                          </span>
                        )
                      ) : (
                    ...
                    {openDropdownMessageId === msg._id &&
                      ...
                      !msg.isDeleted && (
                        <div
                          ...
                          } top-0 mt-6 bg-gray-50 shadow-md rounded-lg border z-10`}
                        >
                          <button
                            onClick={() => {
      ...
      {/* Input Section */}
      {currentConversation &&
      currentConversation.otherParticipant?.isDeleted ? (
