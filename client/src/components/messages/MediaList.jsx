...
const MediaList = () => {
    ...
    if (socket && currentConversation?._id) {
      ...
      const handleNewFile = (data) => {
        if (currentConversation?._id === data.conversationId) {
          ...
          const isDuplicate = (list, file) =>
            list.some((f) => f._id === file._id);

          if (newFichier.type.startsWith("image")) {
            setImages((prev) => {
              if (!isDuplicate(prev, newFichier)) {
                return [...prev, newFichier].filter((f) =>
  ...
  useEffect(() => {
    if (Array.isArray(fichiers)) {
      ...
      setOtherFiles(
        fichiers.filter((fichier) => !fichier.type.startsWith("image"))
      );
    } else {
      setImages([]);
      setOtherFiles([]);
  ...
  const handleDeleteFile = async (fichierId) => {
    try {
      ...
      const fileToDelete = [...images, ...otherFiles].find(
        (f) => f._id === fichierId
      );
  ...
  if (loading) {
    return (
      <div className="h-full w-[250px] flex-shrink-0 flex flex-col border-gray-200 border-2 bg-gray-50 rounded-lg ml-2 shadow-md overflow-hidden text-center">
  ...
  if (!currentConversation) {
    return (
      <div className="h-full w-[250px] flex-shrink-0 flex flex-col bg-gray-50 rounded-lg ml-2 shadow-md overflow-hidden text-center text-gray-500 justify-center">
        Sélectionnez une conversation pour afficher les medias.
      </div>
    );
  }

  // Sécurité supplémentaire dans le rendu
  if (!Array.isArray(images) || !Array.isArray(otherFiles)) return null;

  return (
    <div className="h-full w-[250px] flex-shrink-0 flex flex-col border-gray-200 border-2 bg-gray-50 rounded-lg ml-2 shadow-md overflow-hidden">
