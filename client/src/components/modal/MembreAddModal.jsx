...
const MembreAddModal = ({ onSelect, onClose}) => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        if (data?.users || Array(data.users)) {
          setUsers(data?.users || [])
          setFilteredUsers(data?.users || [])
        } else {
          console.error("Aucune liste recuperer dans modal membre")
        }
      } catch (error) {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = users.filter(
      (user) => 
        user.pseudo.toLowerCase().includes(value) || user.nom.toLowerCase().includes(value) || user.email.toLowerCase().includes(value)
    )
  ...
  return (
    <div className="max-h-[460px] min-w-[300px] flex flex-col bg-gray-50 border-black rounded-lg p-3 shadow-md overflow-hidden">
      ...
      {filteredUsers.length > 0 ? (
        <div className="flex-1 mb-3 overflow-y-auto pl-1 pr-1 custom-scrollbar">
            ...
            >
              ...
              <div className="ml-3 flex-1">
                <div className="flex justify-between items-center">
                  <span className="font-medium text-sm text-gray-600 truncate">{user.nom}</span>
                </div>
